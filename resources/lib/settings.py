...
class Settings():
    ...
    @staticmethod
    def getSleepRemindersStart():
        # Make sure reminders are enabled
        if not Settings.displaySleepReminders():
            return -1

        return int(float(ADDON.getSetting("reminderStart")))

    @staticmethod
    def getSleepRemindersInterval():
        # Make sure reminders are enabled
        if not Settings.displaySleepReminders():
            return -1

        return int(float(ADDON.getSetting("reminderInterval")))
